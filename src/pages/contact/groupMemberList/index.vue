<template>
  <div class="page_container">
    ...
    <virtual-list
      class="my_scrollbar flex-1 overflow-y-auto"
      @tobottom="loadmore"
      :data-key="'userID'"
      :data-sources="renderData"
    ...
    <CheckedFooter
      v-if="showCheck"
      :all-checked-list="allCheckedList"
...
import { AllowType, GroupMemberRole } from '@openim/client-sdk'
...
const showCheckType = [MemberListActionEnum.CallInvite, MemberListActionEnum.Kickout]

const { fetchState, getMemberData } = useGroupMemberList(
  pageState.groupID,
)
...
const allCheckedList = computed(() =>
  fetchState.groupMemberList.filter((member) =>
    checkedMemberIDList.value.includes(member.userID),
  ),
)
...
const renderData = computed(() => {
  if (history.state.action === MemberListActionEnum.Transfer) {
    return fetchState.groupMemberList.filter(
      (member) => member.roleLevel !== GroupMemberRole.Owner,
    )
  }
